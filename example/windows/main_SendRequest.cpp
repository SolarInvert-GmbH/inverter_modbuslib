...
int main(int argc, char *argv[])
{
    ...
    const std::chrono::microseconds waitUntil =
        timeout == 0 ? std::chrono::microseconds{0} : catta::windows::Time::get() + std::chrono::milliseconds(timeout);
    ...
    const auto logConnection = [isVerbose](catta::windows::Uart &uart, bool &boolean, const char *name, const std::chrono::microseconds now)
    {
    ...
    while (!end)
    {
        const auto now = catta::windows::Time::get();
        ...
        if (!somethingHappend) catta::windows::Time::sleep(1us);
    }
