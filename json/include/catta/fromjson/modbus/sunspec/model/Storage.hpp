...
class Parser<catta::modbus::sunspec::model::Storage>
{
  public:
    ...
    [[nodiscard]] constexpr std::tuple<Error, catta::parser::InputHandled> read(const Input& input) noexcept
    {
        ...
        switch (_state)
        {
            ...
            case HUB + 1:
                return input == catta::json::Token::character('w') ? jump(HUB + 2) : error();
            case HUB + 2:
                return input == catta::json::Token::character('C') ? jump(HUB + 3) : error();
            case HUB + 3:
                return input == catta::json::Token::character('h') ? jump(HUB + 4) : error();
            case HUB + 4:
                return input == catta::json::Token::character('a') ? jump(HUB + 5) : error();
            case HUB + 5:
                return input == catta::json::Token::character('R')   ? jump(HUB + 11)
                       : input == catta::json::Token::character('M') ? jump(HUB + 6)
                                                                     : error();
            case HUB + 6:
                return input == catta::json::Token::character('a') ? jump(HUB + 7) : error();
            case HUB + 7:
                return input == catta::json::Token::character('x') ? jump(HUB + 8) : error();
            case HUB + 8:
                return input == catta::json::Token::closeString() ? jump(HUB + 9) : error();
            case HUB + 9:
                return !_wChaMaxParser.state().isStart() ? error() : input == catta::json::Token::colon() ? next() : error();
            case HUB + 10:
                return handle(_wChaMaxParser);
            case HUB + 11:
            ...
            case HUB + 15:
                return !_wChaRateParser.state().isStart() ? error() : input == catta::json::Token::colon() ? next() : error();
            case HUB + 16:
                return handle(_wChaRateParser);
            case TAIL + 0:
    ...
    {
        return _state == DONE ? Output::create(_wChaMaxParser.data(), _wChaRateParser.data()) : Output::empty();
    }
  ...
  private:
    std::uint8_t _state;
    Parser<catta::modbus::sunspec::ValueU16> _wChaMaxParser;
    Parser<catta::modbus::sunspec::ValueU16> _wChaRateParser;
