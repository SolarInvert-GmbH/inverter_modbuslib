...
{
template <>
class Parser<catta::modbus::sunspec::ValueS32>
{
  public:
    using Error = catta::state::DefaultError;
    using Input = catta::json::Token;
    using Output = catta::modbus::sunspec::ValueS32;
    [[nodiscard]] constexpr std::tuple<Error, catta::parser::InputHandled> read(const Input& input) noexcept
    {
        ...
        const auto handle = [this, stay, input, error]()
        {
            ...
            if (number < -2'147'483'648 || number > +2'147'483'647) return error();
            _data = static_cast<std::int32_t>(number);
            _state++;
            return stay();
        };
        switch (_state)
