...
{
template <>
class Parser<catta::modbus::sunspec::ScaledValueS16>
{
  public:
    using Error = catta::state::DefaultError;
    using Input = catta::json::Token;
    using Output = catta::modbus::sunspec::ScaledValueS16;
    [[nodiscard]] constexpr std::tuple<Error, catta::parser::InputHandled> read(const Input& input) noexcept
    {
        ...
        switch (_state)
        {
            ...
            case HUB + 1:
                return input == catta::json::Token::character('v')   ? jump(HUB + 9)
                       : input == catta::json::Token::character('s') ? jump(HUB + 2)
                                                                     : error();
            case HUB + 2:
            ...
            case HUB + 4:
                return input == catta::json::Token::character('l') ? jump(HUB + 5) : error();
            case HUB + 5:
                return input == catta::json::Token::character('e') ? jump(HUB + 6) : error();
            case HUB + 6:
                return input == catta::json::Token::closeString() ? jump(HUB + 7) : error();
            case HUB + 7:
            ...
            case HUB + 10:
                return input == catta::json::Token::character('l') ? jump(HUB + 11) : error();
            case HUB + 11:
                return input == catta::json::Token::character('u') ? jump(HUB + 12) : error();
            case HUB + 12:
    ...
    {
        return _state == DONE ? Output::create(_valueParser.data(), _scaleParser.data()) : Output::empty();
    }
  ...
  private:
    ...
    Parser<catta::modbus::sunspec::ValueS16> _valueParser;
    static constexpr std::uint8_t START = 0;
    static constexpr std::uint8_t HUB = START + 1;
    static constexpr std::uint8_t TAIL = HUB + 16;
