...
{
template <>
class Parser<catta::modbus::sunspec::ScaledValue2U16>
{
  public:
    using Error = catta::state::DefaultError;
    using Input = catta::json::Token;
    using Output = catta::modbus::sunspec::ScaledValue2U16;
    [[nodiscard]] constexpr std::tuple<Error, catta::parser::InputHandled> read(const Input& input) noexcept
    {
        ...
        switch (_state)
        {
            ...
            case HUB + 1:
                return input == catta::json::Token::character('s')   ? jump(HUB + 11)
                       : input == catta::json::Token::character('m') ? jump(HUB + 2)
                                                                     : error();
            case HUB + 2:
                return input == catta::json::Token::character('i')   ? jump(HUB + 7)
                       : input == catta::json::Token::character('a') ? jump(HUB + 3)
                                                                     : error();
            case HUB + 3:
                return input == catta::json::Token::character('x') ? jump(HUB + 4) : error();
            case HUB + 4:
                return input == catta::json::Token::closeString() ? jump(HUB + 5) : error();
            case HUB + 5:
                return !_maxParser.state().isStart() ? error() : input == catta::json::Token::colon() ? next() : error();
            case HUB + 6:
                return handle(_maxParser);
            case HUB + 7:
                return input == catta::json::Token::character('n') ? jump(HUB + 8) : error();
            case HUB + 8:
                return input == catta::json::Token::closeString() ? jump(HUB + 9) : error();
            case HUB + 9:
                return !_minParser.state().isStart() ? error() : input == catta::json::Token::colon() ? next() : error();
            case HUB + 10:
                return handle(_minParser);
            case HUB + 11:
                return input == catta::json::Token::character('c') ? jump(HUB + 12) : error();
            case HUB + 12:
                return input == catta::json::Token::character('a') ? jump(HUB + 13) : error();
            case HUB + 13:
                return input == catta::json::Token::character('l') ? jump(HUB + 14) : error();
            case HUB + 14:
    ...
    {
        return _state == DONE ? Output::create(_minParser.data(), _maxParser.data(), _scaleParser.data()) : Output::empty();
    }
  ...
  private:
    std::uint8_t _state;
    Parser<catta::modbus::sunspec::ValueU16> _maxParser;
    Parser<catta::modbus::sunspec::ValueU16> _minParser;
