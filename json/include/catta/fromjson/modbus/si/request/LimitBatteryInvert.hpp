...
class Parser<catta::modbus::si::request::LimitBatteryInvert>
{
  public:
    ...
    [[nodiscard]] constexpr std::tuple<Error, catta::parser::InputHandled> read(const Input& input) noexcept
    {
        ...
        switch (_state)
        {
            ...
            case HUB + 1:
                return input == catta::json::Token::character('p') ? jump(HUB + 2) : error();
            case HUB + 2:
            ...
            case HUB + 5:
                return input == catta::json::Token::character('f') ? jump(HUB + 6) : error();
            case HUB + 6:
            ...
            case HUB + 9:
                return input == catta::json::Token::closeString() ? jump(HUB + 10) : error();
            case HUB + 10:
                return !_pMaxfeedParser.state().isStart() ? error() : input == catta::json::Token::colon() ? next() : error();
            case HUB + 11:
                return handle(_pMaxfeedParser);
            case TAIL + 0:
    ...
    [[nodiscard]] constexpr Output data() const noexcept { return _state == DONE ? Output::create(_pMaxfeedParser.data()) : Output::empty(); }
  ...
  private:
    std::uint8_t _state;
    Parser<catta::modbus::sunspec::ValueU8<0, 100>> _pMaxfeedParser;
    static constexpr std::uint8_t START = 0;
    static constexpr std::uint8_t HUB = START + 1;
    static constexpr std::uint8_t TAIL = HUB + 12;
