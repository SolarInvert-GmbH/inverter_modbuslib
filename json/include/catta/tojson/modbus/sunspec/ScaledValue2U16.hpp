...
{
template <>
class Serializer<catta::modbus::sunspec::ScaledValue2U16>
{
  public:
    using Error = catta::state::DefaultError;
    using Input = catta::modbus::sunspec::ScaledValue2U16;
    ...
    [[nodiscard]] constexpr std::tuple<Error, catta::parser::InputHandled> read(const Input& input) noexcept
    {
        ...
        switch (_state)
        {
            ...
            case MIN + 6:
                return handle(_minSerializer, input.min(), catta::json::Token::comma());
            case MAX + 0:
                return next(catta::json::Token::openString());
            case MAX + 1:
                return next(catta::json::Token::character('m'));
            ...
            case MAX + 5:
                return next(catta::json::Token::colon());
            case MAX + 6:
                return handle(_maxSerializer, input.max(), catta::json::Token::comma());
            case SCALE + 0:
                return next(catta::json::Token::openString());
            case SCALE + 1:
  ...
  private:
    ...
    Serializer<catta::modbus::sunspec::ValueU16> _minSerializer;
    Serializer<catta::modbus::sunspec::ValueU16> _maxSerializer;
    Serializer<catta::modbus::sunspec::ScaleFactor> _scaleSerializer;
    static constexpr std::uint8_t START = 0;
    static constexpr std::uint8_t MIN = START + 1;
    static constexpr std::uint8_t MAX = MIN + 7;
    static constexpr std::uint8_t SCALE = MAX + 7;
