...
list()
{
    status "Grafana user list"
    LINES="$(curl -s -u "${VAR_USER_PASSWORD}" -H "Content-Type: application/json" http://localhost:3000/api/users | jq -r '.[] | "\(.id)\t\(.uid)\t\(.name)\t\(.login)\t\(.email)\t\(.isAdmin)\t\(.lastSeenAtAge)\t\(.authLabels)"')"
    status_multy "${LINES}"
}
