...
class Uart
{
  public:
        ...
        const std::chrono::microseconds now, const std::optional<std::uint8_t> received, const catta::modbus::Token send)
    {
        ...
        const auto sendMiddle = [this, send, computeCrc](const std::uint8_t state)
        {
            _state = state;
            computeCrc(send.value());
            return Tuple{State::send(), Token::empty(), Byte{send.value()}, Handled::yes()};
        };
        const auto sendFirst = [this, sendMiddle]()
        {
            _crc = 0xffff;
            return sendMiddle(SEND_FUNCTION);
        };
        const auto sendLength = [this, sendMiddle, send]()
        {
            _index = 0;
            _length = send.value();
            return sendMiddle(_length == 0 ? SEND_CRC0 : SEND_DATA);
        };
        const auto sendData = [this, sendMiddle]()
        {
            _index++;
            return sendMiddle(_index >= _length ? SEND_CRC0 : SEND_DATA);
        };
        ...
        {
            _crc = 0xffff;
            return receiveBase(RECEVIE_WAIT_FOR_FUNCTION, Token::address(received.value()));
        };
        const auto receiveFunction = [receiveBase, received]() { return receiveBase(RECEVIE_WAIT_FOR_LENGTH, Token::function(received.value())); };
        const auto receiveLength = [this, receiveBase, received, error]()
        {
            ...
            _index = 0;
            return receiveBase(RECEVIE_WAIT_FOR_DATA, Token::length(received.value()));
        };
        ...
        const auto receiveCrc1 = [this, now, received, error]()
        {
            ...
            _waitUntil = now + dataTimeout;
            return Tuple{State::receive(), Token::end(), Byte{}, Handled::no()};
        };
        ...
        switch (_state)
        {
            case IDLE:
                return received                  ? error(ERROR_RECEIVE_WITHOUT_REUQUEST)
                       ...
                       : send.type().isAddress() ? sendFirst()
                                                 : error(ERROR_SEND_INVALID);
            case SEND_FUNCTION:
                return received                   ? error(ERROR_RECEIVE_WITHOUT_REUQUEST)
                       : send.isEmpty()           ? stay(State::send())
                       : send.type().isFunction() ? sendMiddle(SEND_LENGTH)
                                                  : error(ERROR_SEND_INVALID);
            case SEND_LENGTH:
                return received                 ? error(ERROR_RECEIVE_WITHOUT_REUQUEST)
                       : send.isEmpty()         ? stay(State::send())
                       : send.type().isLength() ? sendLength()
                                                : error(ERROR_SEND_INVALID);
            case SEND_DATA:
                return received               ? error(ERROR_RECEIVE_WITHOUT_REUQUEST)
                       : send.isEmpty()       ? stay(State::send())
  ...
  private:
    ...
    static constexpr std::uint8_t SEND_LENGTH = SEND_FUNCTION + 1;
    static constexpr std::uint8_t SEND_DATA = SEND_LENGTH + 1;
    ...
    static constexpr std::uint8_t RECEVIE_WAIT_FOR_CRC0 = RECEVIE_WAIT_FOR_DATA + 1;
    ...
    static constexpr std::uint8_t ERROR_RECEIVE_WRONG_CRC = ERROR_RECEIVE_LENGTH_NOT_VALID + 1;
