...
static bool checkCases(catta::test::Test<OUTPUT>& test)
{
    ...
    const TokenVec sendInput1 = {Token::start(),    Token::function(0x10), Token::data(0x9c), Token::data(0x40),
                                 Token::data(0x00), Token::data(0x01),     Token::end()};
    const ByteVec sendOutput1 = {0x01, 0x10, 0x9c, 0x40, 0x00, 0x01, 0x2e, 0x4d};
    ...
    const auto checkNormal = [&now, &uart, &test](const std::uint8_t modbusId, const TokenVec& sendInput, const ByteVec& sendOutputExpected,
                                                  ...
                                                  const catta::modbus::MasterUartState error = catta::modbus::MasterUartState::empty())
    {
        ...
        const auto errorCase = [&test, &uart, &now, error, modbusId](const Token inToken, const Byte inByte)
        {
            {
                const auto [state, token, send, handled] = uart.work(now, inByte, inToken, modbusId);
                if (debug)
                    test.status("        [" + catta::tostring::toString(state) + "," + catta::tostring::toString(token) + "," + printByte(send) +
                                ...
                                catta::tostring::toString(inToken) + "," + catta::tostring::toString(catta::Hexadecimal(modbusId)) + ")");
            ...
            const auto [state, token, send, handled] = uart.work(now, Byte(), Token::empty(), modbusId);
            if (debug)
                test.status("        [" + catta::tostring::toString(state) + "," + catta::tostring::toString(token) + "," + printByte(send) + "," +
                            catta::tostring::toString(handled) + "] = uart.work(" + printTime(now) + ", {}, " +
                            catta::tostring::toString(Token::empty()) + "," + catta::tostring::toString(catta::Hexadecimal(modbusId)) + ")");
            ...
            if (send)
            {
                test.failed("Expected " + printByte(Byte()) + " as received output,");
                test.failed("but got  " + printByte(send) + ".");
                return false;
            }
            return true;
        ...
        {
            const auto handle = [&now, &uart, &test, &s, &sendOutput, modbusId](const catta::parser::InputHandled handledExpected)
            {
                const auto [state, token, send, handled] = uart.work(now, Byte{}, s, modbusId);
                if (debug)
                    test.status("        [" + catta::tostring::toString(state) + "," + catta::tostring::toString(token) + "," + printByte(send) +
                                "," + catta::tostring::toString(handled) + "] = uart.work(" + printTime(now) + ", {}, " +
                                catta::tostring::toString(s) + "," + catta::tostring::toString(catta::Hexadecimal(modbusId)) + ")");
        ...
        for (std::size_t i = 0; i < receiveInput.size(); i++)
        {
            ...
            const auto [state, token, send, handled] = uart.work(now, receiveInput[i], Token::empty(), modbusId);
            if (debug)
                test.status("        [" + catta::tostring::toString(state) + "," + catta::tostring::toString(token) + "," + printByte(send) + "," +
        ...
        {
            test.failed("Expected " + printTokenVec(receiveOutputExpected) + " as received output,");
